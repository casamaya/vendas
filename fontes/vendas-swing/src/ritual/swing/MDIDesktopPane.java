 ...
 * By Gerald Nunn, JavaWorld.com, 05/25/01
 */
public class MDIDesktopPane extends JDesktopPane {
    ...
    public void tileFrames(JInternalFrame allFrames[]) {
        manager.setNormalSize();        
    ...
    public void tileFrames() {
        tileFrames(getAllFrames());
    }
    Integer frameHeight;
    Integer frameWidth;
    public void tileFrames(List<JInternalFrame> lista) {
        manager.setNormalSize();    
        if (frameHeight == null) {
            frameHeight = getBounds().height/lista.size();
        }
        if (frameWidth == null) {
            frameWidth = getBounds().width;
        }
        int y = 0;
        for (JInternalFrame frame : lista) {
            if (frame.isResizable())
