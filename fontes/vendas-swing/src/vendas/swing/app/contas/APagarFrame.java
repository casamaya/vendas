 ...
 * @author  p993702
 */
public class APagarFrame extends TableViewFrame {
    
    Vendedor vendedor;

    public APagarFrame(ServiceTableModel tableModel) throws Exception {
        super("Contas a pagar", tableModel);
    }
    ...
    @Override
    public void insert() {
        if (TApplication.getInstance().isGrant("INCLUIR_CONTA_APAGAR"))
            super.insert();
    }
    
    @Override
    public void remove() {
        if (!TApplication.getInstance().isGrant("EXCLUIR_CONTA_APAGAR"))
            return;
    ...
    public void insertRow() {
        APagar conta = new APagar();
        conta.setDtVencimento(new Date());
        conta.setValor(new BigDecimal(0));
        APagarEditPanel editPanel = new APagarEditPanel();
    ...
    public void editRow() {
        ...
        APagar conta = (APagar) vtm.getObject(getSelectedRow());
        ...
        edtDlg.setEditPanel(editPanel, !TApplication.getInstance().isGrant("ALTERAR_CONTA_APAGAR"));
        while (edtDlg.edit(conta)) {
            try {
                vtm.updateRow(conta);
                break;
    ...
    private boolean isFiltroValido(CompromissoFilter value) {
        if (   (value.getConta() == null)
            && (value.getDtIni() == null)
            && (value.getDtIniPgto() == null)
            && (value.getGrupo() == null)
            && (value.getSituacao() == 0)
            && (value.getTipoPgto() == null) )
            return false;
        else
            return true;
    }
    ...
    public void filter() {
        ...
        filterPanel.setContas(VendasUtil.getAPagarListModel());
        filterPanel.setGrupo(VendasUtil.getAPagarMasterListModel());
        ...
        while (edtDlg.edit(filtro)) {
            try {
                if (!isFiltroValido((CompromissoFilter)filtro)) {
                    Messages.errorMessage("Informe um parâmetro");
                    continue;
                }
    ...
    public void showReport() {
        ...
        if (n == 2) {
            ...
            url = getClass().getResource("/vendas/report/CompromissoReport.jasper");
        } else {
            url = getClass().getResource("/vendas/report/DetalhesReport.jasper");
            try {
                CompromissoDao dao = (CompromissoDao) getTableModel().getDao();
