 ...
 * @author Sam
 */
public class MapaVisitasView extends FilterFrame {

    TApplication app = TApplication.getInstance();

    public MapaVisitasView() {
        super();
        setTitle(app.getResourceString("mapaVisitasTitle"));
    ...
    public void execute() {
        ...
        try {
            List lista = pedidoDao.findMapaVisitas(cliente);
            if ((lista == null) || lista.size() == 0) {
                Messages.errorMessage(app.getResourceString("noDataReport"));
            } else {
                showReport(app.getResourceString("mapaVisitasTitle"), cliente.getTitle(), Constants.JRROTATIVIDADE, lista);
            }
        } catch (Exception e) {
