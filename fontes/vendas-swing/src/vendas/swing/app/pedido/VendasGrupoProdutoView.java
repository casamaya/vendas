 ...
 * @author Sam
 */
public class VendasGrupoProdutoView extends FilterFrame {

    TApplication app = TApplication.getInstance();

    public VendasGrupoProdutoView() {
        super();
        setTitle(app.getResourceString("vendasGrupoProdutoTitle"));
    ...
    public void execute() {
        ...
        try {
            List<ResumoVendasUnidade> lista = pedidoDao.findVendasGrupoProduto(pedidoFilter);
            if ((lista == null) || lista.isEmpty()) {
                ...
                Map model = TApplication.getInstance().getDefaultMap(app.getResourceString("vendasGrupoProdutoTitle"), pedidoFilter.getTitle());
                model.put("valorTotal", totalComissao);
                Reports.showReport(getClass().getResource(Constants.JRVENDASGRUPOPRODUTO), model, lista);
            }
        } catch (Exception e) {
